...
public class EarthSurge extends WizardStyle {
    ...
    public static final int COOLDOWN_ABILITY_TICKS = 18;
        ...
        if (WizardUtil.handleManaCostAndCooldown(p, this, MANA_COST)) {
            ...
            for (int i = 0; i < 6; i++) {
                Bukkit.getScheduler().scheduleSyncDelayedTask(CoronaCraft.instance, () -> {
                    if (valid(p) && AbilityUtil.notInSpawn(p)) {
                        sendProjectile(p);
                    }
                }, i * 3);
            }
    ...
    private void sendProjectile(final Player p) {
    ...
    private boolean valid(Player p) {
        ...
        return zone != null && !zone.isReinitializing();
    }
