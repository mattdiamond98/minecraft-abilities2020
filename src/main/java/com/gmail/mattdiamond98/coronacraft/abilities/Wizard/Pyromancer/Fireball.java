        ...
        super(
                ...
                new Ability("Fireball", Material.FIRE_CHARGE) {
                    ...
                    public void onProjectileHit(ProjectileHitEvent e) {
                        if (e.getEntityType() == EntityType.FIREBALL && e.getEntity().getShooter() instanceof Player&& Warzone.getZoneByLocation(e.getEntity().getLocation())!=null) {
                            e.getEntity().getWorld().createExplosion(
                                    e.getEntity().getLocation(), 2.2F, true, false, (Player) e.getEntity().getShooter()
                            );
                            e.getEntity().remove();
                        }
                        else if(e.getEntityType()==EntityType.SMALL_FIREBALL && e.getEntity().getShooter() instanceof Player && Warzone.getZoneByLocation(e.getEntity().getLocation())==null){
                            e.getEntity().remove();
                        }else if(e.getEntityType() == EntityType.FIREBALL && e.getEntity().getShooter() instanceof Player&& Warzone.getZoneByLocation(e.getEntity().getLocation())==null){
                            e.getEntity().remove();
                        }
                    }
                    @EventHandler
