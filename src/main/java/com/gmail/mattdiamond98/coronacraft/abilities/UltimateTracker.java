...
import com.gmail.mattdiamond98.coronacraft.event.UltimateEndEvent;
...
public class UltimateTracker {
    ...
    public static HashMap<UUID, UltimateAbility> activeUlts=new HashMap<>();
    ...
    public static void removeProgress(UUID player) {
        ...
        if (activeUlts.containsKey(player)) {
            UltimateEndEvent e=new UltimateEndEvent(Bukkit.getPlayer(player), activeUlts.get(player));
            Bukkit.getPluginManager().callEvent(e);
            activeUlts.remove(player);
    ...
    public static boolean isUltimateActive(Player player, UltimateAbility ultimateAbility){
        return activeUlts.containsKey(player.getUniqueId()) && activeUlts.get(player.getUniqueId()).getName().equals(ultimateAbility.getName());
    }
