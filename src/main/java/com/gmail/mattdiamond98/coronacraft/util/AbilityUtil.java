...
import com.gmail.mattdiamond98.coronacraft.abilities.Anarchist.Buddies;
import com.gmail.mattdiamond98.coronacraft.abilities.Anarchist.TNTTrail;
import com.gmail.mattdiamond98.coronacraft.abilities.Berserker.PureHatred;
...
import com.gmail.mattdiamond98.coronacraft.abilities.Fighter.GigaSlash;
...
import com.gmail.mattdiamond98.coronacraft.abilities.Wizard.Geomancer.MeteorRain;
import com.gmail.mattdiamond98.coronacraft.abilities.Wizard.Pyromancer.DragonfireHelix;
import com.gmail.mattdiamond98.coronacraft.abilities.Wizard.Thaumaturge.Blizzard;
import com.gmail.mattdiamond98.coronacraft.abilities.Wizard.Thaumaturge.Typhoon;
...
public final class AbilityUtil {
    ...
    public static List<PotionEffectType> debuffs= Arrays.asList(new PotionEffectType[]{PotionEffectType.WEAKNESS, PotionEffectType.SLOW, PotionEffectType.SLOW_DIGGING, PotionEffectType.POISON, PotionEffectType.INCREASE_DAMAGE, PotionEffectType.BLINDNESS, PotionEffectType.LEVITATION, PotionEffectType.WITHER, PotionEffectType.HARM, PotionEffectType.CONFUSION, PotionEffectType.HUNGER, PotionEffectType.UNLUCK, PotionEffectType.BAD_OMEN});
    public static UltimateAbility[] ALL_ULTS={new TNTTrail(), new UndyingFrenzy(), new SpacetimePortal(), new Omnislash(), new StormGodsWrath(), new DivineAura(),
            new ShadowStride(), new ArrowStorm(), new InstinctiveHunter(), new DesolationFist(), new BossAbility(), new WizardUltimateAbility(),new Typhoon(),
            new DragonfireHelix(), new MeteorRain(), new Buddies(), new GigaSlash(), new DeathsCarnage(), new PureHatred()};

    public static boolean isPlayerInAir(Player p){
        return p.getLocation().add(0, -1, 0).getBlock().getType().isAir();
    }
    ...
    public static void sendWorldBorderPacket(Player player) {
    ...
    public static Vector getVectorBetweenTwoLocations(Location p1Loc, Location p2Loc) {

        Vector v1 = new Vector(p1Loc.getBlockX(), p1Loc.getBlockY(), p1Loc.getBlockZ());
        Vector v2 = new Vector(p2Loc.getBlockX(), p2Loc.getBlockY(), p2Loc.getBlockZ());
    ...
    public static UltimateAbility getUltByName(String name){
        for(UltimateAbility ua:ALL_ULTS){
            if(name.contains(ua.getName())){
                return ua;
            }
    ...
    public static void applyStunEffect(LivingEntity player, int ticks) {
        new PotionEffect(PotionEffectType.JUMP, ticks, -1).apply(player);
        ...
        p.sendTitle("", ChatColor.RED + "STUNNED", 0, ticks, 0);
        p.sendMessage(ChatColor.RED + "You have been stunned!.");}
    }
    ...
    public static void regenerateItem(Player p, ItemStack item, int max, int increment) {
        if (notInSpawn(p)) {
            int count = getTotalCount(p, item.getType());
            int newCount = Math.min(count + increment, max);
            if (count < newCount) {
