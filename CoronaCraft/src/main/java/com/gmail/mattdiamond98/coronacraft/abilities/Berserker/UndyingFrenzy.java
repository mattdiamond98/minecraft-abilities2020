...
public class UndyingFrenzy extends UltimateAbility {
    ...
    public static final int DURATION_SECONDS = 35;
    ...
    public void onPlayerResurrect(EntityResurrectEvent e) {
        if (e.getEntity() instanceof Player) {
            ...
            if (UltimateTracker.isUltimateActive(p) && UltimateTracker.getLoadout(p) == Loadout.BERSERKER) {
                Bukkit.getScheduler().scheduleSyncDelayedTask(CoronaCraft.instance, () -> p.setHealth(20), 1);
    ...
    public void onEnd(CoolDownEndEvent e) {
        if (e.getItem() == Material.NETHER_STAR && UltimateTracker.getLoadout(e.getPlayer()) == Loadout.BERSERKER) {
            if (e.getPlayer().getInventory().getItemInOffHand().getType() == Material.TOTEM_OF_UNDYING) {
                e.getPlayer().getInventory().setItemInOffHand(null);
            } else if (e.getPlayer().getItemOnCursor().getType() == Material.TOTEM_OF_UNDYING) {
                e.getPlayer().setItemOnCursor(null);
            } else {
                e.getPlayer().getInventory().remove(Material.TOTEM_OF_UNDYING);
            }
