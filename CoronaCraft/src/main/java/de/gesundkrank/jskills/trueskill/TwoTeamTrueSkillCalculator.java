...
public class TwoTeamTrueSkillCalculator extends SkillCalculator {
    ...
    private static void updatePlayerRatings(GameInfo gameInfo,
        ...
        for (Rating r : otherTeam.values()) {
            otherTeamMeanSum += r.getMean();
        }

        double sum = 0;
        for (Rating r : selfTeam.values()) {
            sum += square(r.getStandardDeviation());
        }
        for (Rating r : otherTeam.values()) {
            sum += square(r.getStandardDeviation());
        }
        ...
        for (Entry<IPlayer, Rating> teamPlayerRatingPair : selfTeam.entrySet()) {
            ...
            double
                    meanMultiplier =
                    (square(previousPlayerRating.getStandardDeviation()) + tauSquared) / c;
            double
                    stdDevMultiplier =
            ...
            double newStdDev =
                    Math.sqrt(
                            (square(previousPlayerRating.getStandardDeviation()) + tauSquared) * (1
                                                                                                  -
                                                                                                  w
                                                                                                  * stdDevMultiplier));
    ...
    public double calculateMatchQuality(GameInfo gameInfo, Collection<ITeam> teams) {
        ...
        for (Rating r : team1) {
            team1StdDevSquared += square(r.getStandardDeviation());
        }
        ...
        for (Rating r : team2) {
            team2SigmaSquared += square(r.getStandardDeviation());
        }
